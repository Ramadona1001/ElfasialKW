...
<div class="page-section">      
  <div class="container wide">
    ...
    <div class="content-element">
            ...
            <!-- Info Boxes -->
            @php($fromChoiceItemImages = \DB::select('select * from fromchoices_gallery where fromchoice_id = '.$items->id))
            <div class="row">
            ...
            <hr>
            <p style="font-weight:bold;font-size:25px;">{{ $items->name }}</p>
            
            <p style="font-weight:bold;font-size:18px;">{{ $items->desc }}</p>
            <p style="font-weight:bold;font-size:25px;">@lang('tr.Price'): {{ $items->inventory->price.' '.$system_currency }}</p>

            <hr>

            <form action="{{ route('frontend_add_to_cart') }}" method="post">
                @csrf
                <input type="hidden" name="fromchoice_id" value="{{ $items->id }}">  
                <button type="submit" class="addToCart" style="display: block; margin-left: auto; margin-right: auto; width: 300px;"><i class="fa fa-cart-plus"></i><a > @lang('tr.add to cart')</a></button> 
              </form>
